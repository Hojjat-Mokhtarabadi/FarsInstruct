...
class RougeScorer(scoring.BaseScorer):
    ...
    def score(self, target, prediction):
        ...
        target_tokens = tokenize.tokenize(target, self._stemmer, lang=self.lang)
        prediction_tokens = tokenize.tokenize(prediction, self._stemmer, lang=self.lang)
        result = {}
        ...
        for rouge_type in self.rouge_types:
            if rouge_type == "rougeL":
                ...
                target_tokens_list = [
                    tokenize.tokenize(s, self._stemmer, lang=self.lang) for s in get_sents(target)]
                prediction_tokens_list = [
                    tokenize.tokenize(s, self._stemmer, lang=self.lang) for s in get_sents(prediction)]
